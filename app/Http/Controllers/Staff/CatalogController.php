...
class CatalogController extends Controller
{
    ...
    public function postCatalog(Request $request)
    {
        ...
        if ($catalog) {
            return redirect()->route('catalogs')
                ->with($this->toastr->error('Catalog '.$catalog->name.' is already in database', trans('toastr.error'), ['options']));
        }
        $catalog = new Catalog();
        ...
        return redirect()->route('getCatalog')
            ->with($this->toastr->success('Catalog '.$request->input('catalog').' has been successfully added', trans('toastr.success'), ['options']));
    }
    ...
    public function editCatalog(Request $request, $catalog_id)
    {
        ...
        if (! $catalog) {
            return redirect()->route('getCatalog')
                ->with($this->toastr->error('Catalog '.$request->input('catalog').' is not in our DB!', trans('toastr.error'), ['options']));
        }
        $catalog->name = $request->input('catalog');
        $catalog->save();

        return redirect()->route('getCatalog')
            ->with($this->toastr->success('Catalog '.$request->input('catalog').' has been successfully edited', trans('toastr.success'), ['options']));
    }
    ...
    public function deleteCatalog($catalog_id)
    {
        ...
        if (! $catalog) {
            return redirect()->route('getCatalog')
                ->with($this->toastr->error('That Catalog Is Not In Our DB!', trans('toastr.error'), ['options']));
        }
        $catalog->delete();

        return redirect()->route('getCatalog')
            ->with($this->toastr->success('Catalog '.$catalog->name.' has been successfully deleted', trans('toastr.success'), ['options']));
    }
    ...
    public function postCatalogTorrent(Request $request)
    {
        ...
        if ($torrent) {
            return redirect()->route('getCatalogTorrent')->with($this->toastr->error('IMDB# '.$torrent->imdb.' is already in database', trans('toastr.error'), ['options']));
        }
        ...
        return redirect()->route('getCatalogTorrent')->with($this->toastr->success('IMDB# '.$request->input('imdb').' has been successfully added', trans('toastr.success'), ['options']));
    }
